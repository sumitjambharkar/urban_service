import React from 'react'

const page = () => {
  return (
    <div>
    
      <div class="product-detail">
        <div class="product-images">
          <img
            src="https://res.cloudinary.com/dclgpfheh/image/upload/v1734197477/Leonardo_Phoenix_A_highly_detailed_and_realistic_illustration_0_beg9bv.jpg"
            alt="Main Product Image"
            class="main-image"
          />
        </div>
        <div class="product-info">
          <h1> Chandelier Cleaning  : 1Ft x 2Ft </h1>
          <p class="sku">Cleannation is a professional cleaning service provider dedicated to delivering top-quality cleaning solutions for residential, commercial, and industrial spaces. With a focus on efficiency, reliability, and eco-friendly practices, Cleannation ensures every corner of your space is pristine and inviting
          </p>
          <p class="price">₹ 1,500 To ₹ 2,500</p>
       
          <div class="actions_wrraper">
            <div class="actions">
            <button class="add-to-cart">Call Now </button>
            <button class="add-to-cart">Contact Us</button>
            </div>
          </div>
          <div class="additional-info">
            <p>
              <strong>Free Home Visit</strong>
            </p>
            <p>
              <strong>Customer Support No:</strong> 9323969471
            </p>
            <p>
            <details>
                <summary>
                  <strong>We Do </strong>
                </summary>
                <p>cleaning</p>
   
              </details>
            </p>
            <p>
              <strong>Timing:</strong> 1 - 2 Working Hours
            </p>
          </div>
        </div>
      </div>
     

      <div class="product-detail">
        <div class="product-images">
          <img
            src="https://res.cloudinary.com/dclgpfheh/image/upload/v1734197477/Leonardo_Phoenix_A_majestic_crystal_chandelier_hangs_from_a_hi_0_erphci.jpg"
            alt="Main Product Image"
            class="main-image"
          />
        </div>
        <div class="product-info">
          <h1> Chandelier Cleaning  : 3Ft x 4Ft </h1>
          <p class="sku">Cleannation is a professional cleaning service provider dedicated to delivering top-quality cleaning solutions for residential, commercial, and industrial spaces. With a focus on efficiency, reliability, and eco-friendly practices, Cleannation ensures every corner of your space is pristine and inviting
          </p>
          <p class="price">₹ 1,800 To ₹ 2,800</p>
       
          <div class="actions_wrraper">
            <div class="actions">
            <button class="add-to-cart">Call Now </button>
            <button class="add-to-cart">Contact Us</button>
            </div>
          </div>
          <div class="additional-info">
            <p>
              <strong>Free Home Visit</strong>
            </p>
            <p>
              <strong>Customer Support No:</strong> 9323969471
            </p>
            <p>
            <details>
                <summary>
                  <strong>We Do </strong>
                </summary>
                <p>cleaning</p>
   
              </details>
            </p>
            <p>
              <strong>Timing:</strong> 1 - 3 Working Hours
            </p>
          </div>
        </div>
      </div>
      <div class="product-detail">
        <div class="product-images">
          <img
            src="https://res.cloudinary.com/dclgpfheh/image/upload/v1734197477/Leonardo_Phoenix_A_delicate_crystal_chandelier_adorned_with_in_0_pp3liz.jpg"
            alt="Main Product Image"
            class="main-image"
          />
        </div>
        <div class="product-info">
          <h1> Chandelier Cleaning  : 5Ft x 6Ft </h1>
          <p class="sku">Cleannation is a professional cleaning service provider dedicated to delivering top-quality cleaning solutions for residential, commercial, and industrial spaces. With a focus on efficiency, reliability, and eco-friendly practices, Cleannation ensures every corner of your space is pristine and inviting

          </p>
          <p class="price">₹ 2,800 To ₹ 3,500</p>
       
          <div class="actions_wrraper">
            <div class="actions">
            <button class="add-to-cart">Call Now </button>
            <button class="add-to-cart">Contact Us</button>
            </div>
          </div>
          <div class="additional-info">
            <p>
              <strong>Free Home Visit</strong>
            </p>
            <p>
              <strong>Customer Support No:</strong> 9323969471
            </p>
            <p>
            <details>
                <summary>
                  <strong>We Do </strong>
                </summary>
                <p>cleaning</p>
   
              </details>
            </p>
            <p>
              <strong>Timing:</strong> 1 - 4 Working Hours
            </p>
          </div>
        </div>
      </div>
    </div>
  )
}

export default page